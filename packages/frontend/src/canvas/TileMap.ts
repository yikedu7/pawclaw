...
/**
 * Pick the correct Grass.png tile from the Group-A 3×3 small-island set
 * based on the 4 cardinal neighbours (1 = grass, 0 = water/empty).
 *
 * Group A layout (col, row):
 *   (0,0)=TL  (1,0)=T   (2,0)=TR
 *   (0,1)=L   (1,1)=F   (2,1)=R
 *   (0,2)=BL  (1,2)=B   (2,2)=BR
 */
function tileCoord(N: number, E: number, S: number, W: number): [number, number] {
  ...
  return          [1, 1];       // inner / fully surrounded
}
