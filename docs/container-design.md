...
- **Webhook egress — NOTE:** Earlier research described `delivery.mode: "webhook"` in cron/heartbeat config, but this is **not used** in the actual implementation. OpenClaw event delivery to the backend occurs via the WebSocket gateway, not webhook egress. See `config-generator.ts` for the authoritative `openclaw.json` schema.
...
OpenClaw routes LLM output back to the backend via the WebSocket gateway on port 18789 — not via a webhook egress URL in `openclaw.json`. The `openclaw.json` written at container creation contains only `gateway`, `agents.defaults.heartbeat` (with `lightContext: true`), and no `delivery` block. On-chain wallet operations (X402, Onchain OS) remain in the Railway backend process. See `packages/backend/src/runtime/config-generator.ts` for the authoritative config schema.
...
The tick contract has two directions:
1. **Tick ingress:** The backend connects to the OpenClaw WebSocket gateway on port 18789 and sends a tick message. The gateway token (`OPENCLAW_GATEWAY_TOKEN`) authenticates the connection.
2. **Event egress:** OpenClaw delivers LLM results back to the backend via the same WebSocket connection. There is no separate HTTP webhook egress; the `delivery.mode: "webhook"` pattern described in earlier issue #37 research is not used in the actual implementation (confirmed via `openclaw doctor` — `delivery` and `webhooks` are not valid config keys).

The backend's tick loop (in `packages/backend/src/runtime/`) drives the schedule; OpenClaw executes the LLM reasoning and returns structured events via the WebSocket gateway.
...
### Actual openclaw.json written at container creation

The config file is generated by `packages/backend/src/runtime/config-generator.ts` and written to the config bind mount at `/data/pets/{petId}/config/openclaw.json`. Its actual shape is:
...
There is no `delivery`, `cron`, or webhook egress block. Event delivery from the container to the backend occurs via the WebSocket gateway (port 18789), not via HTTP webhook POSTs. The `OPENCLAW_GATEWAY_TOKEN` is injected as a container env var and also set here as `gateway.auth.token`; the backend uses it to authenticate WebSocket connections from the container.
