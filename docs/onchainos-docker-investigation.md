...
onchainos wallet login
  → 调用 OKX API（成功）
  → 尝试用 add_key() 将 session token 存入内核 keyring
...
# swap、send 等签名操作也会失败（私钥存在 keyring 里）
```
...
**安全权衡**：`seccomp=unconfined` 移除了所有系统调用过滤。对于此项目可接受（容器已有内存限制和 restart policy，OKX API key 是最敏感的资产），但长期应考虑只开放 `add_key` 的自定义 seccomp profile 来缩小攻击面。

**延伸影响**：SKILL.md 里所有依赖 onchainos 的技能（wallet、swap、payment）在修复之前对容器内的 LLM 来说是完全不可用的死代码。`seccomp=unconfined` 是让这套架构能跑起来的前提条件。
