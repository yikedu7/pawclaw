...
### R15: PAW token contract redeployed — EIP-3009 (action needed)
...
- **Fix:** New `PawToken` contract in `packages/contracts/` implements full EIP-3009 (`transferWithAuthorization`, `cancelAuthorization`, `authorizationState`, `nonces`). Compile and deploy with `forge script`.
- **Action needed:** Deploy the new contract (see `packages/contracts/README.md`), then update `PAYMENT_TOKEN_ADDRESS` in `.env` and Railway env vars. Re-run `grantRegistrationCredits` for existing pets.
